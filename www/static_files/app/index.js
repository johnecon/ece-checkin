Ext.setup({
	...
	onReady: function () {
		...
		if (!fbAccessToken && !fsAccessToken && !gpAccessToken && !getStarted) {
...
//                        }
			...
			function getFbFsMixedPlacesTab(fbStore, fsStore) {
				...
				var tpl = new Ext.XTemplate(
					'<div><strong class="list-item-property">Facebook:</strong> {fbName} <br/>' +
						'<strong class="list-item-property">Foursquare:</strong> {fsName} <br/>' +
						'<strong class="list-item-property">Distance:</strong> {distance} meters' +
						'<tpl if="communityValidated">  <br/> <strong class="list-item-property"><img src="../resources/img/communityValidatedPlace.png" align="middle"/> -- {counter} votes</strong> </tpl></div>'
				);
				var placeList = {
					itemTpl: tpl,
					selModel: {
			...
			function getFbGpMixedPlacesTab(fbStore, gpStore) {
				...
				var tpl = new Ext.XTemplate(
					'<div><strong class="list-item-property">Facebook:</strong> {fbName} <br/>' +
						'<strong class="list-item-property">Google +:</strong> {gpName} <br/>' +
						'<strong class="list-item-property">Distance:</strong> {distance} meters' +
						'<tpl if="communityValidated">  <br/> <strong class="list-item-property"><img src="../resources/img/communityValidatedPlace.png" align="middle"/> -- {counter} votes</strong> </tpl></div>'
				);
				var placeList = {
			...
			function getFsGpMixedPlacesTab(fsStore, gpStore) {
				...
				var tpl = new Ext.XTemplate(
					'<div><strong class="list-item-property">Foursquare:</strong> {fsName} <br/>' +
						'<strong class="list-item-property">Google +:</strong> {gpName} <br/>' +
						'<strong class="list-item-property">Distance:</strong> {distance} meters' +
						'<tpl if="communityValidated"> <br/>  <strong class="list-item-property"><img src="../resources/img/communityValidatedPlace.png" align="middle"/> -- {counter} votes</strong> </tpl></div>'
				);
                    ...
                    Ext.util.JSONP.request({
                        ...
                        callback: function(result) {
                            ...
                            for (i=0; i<venues.length; i++) {
                                ...
                                places.push({name: venues[i].name, distance: venues[i].location.distance, lat: venues[i].location.lat, lon: venues[i].location.lng, address: venues[i].location.address, category: venues[i].categories.name, iconUrl: venues[i].categories[0].icon.prefix + venues[i].categories[0].icon.sizes[0] + venues[i].categories[0].icon.name, id: venues[i].id}) :
                                places.push({name: venues[i].name, distance: venues[i].location.distance, lat: venues[i].location.lat, lon: venues[i].location.lng, address: venues[i].location.address, category: venues[i].categories.name, id: venues[i].id});
                            }
                            var placeList = {
                                itemTpl: '<div class="contact2"><strong>{name}</strong>, {distance} meters</div>',
                                selModel: {
                                ...
                                Ext.util.JSONP.request({
                                    ...
                                    callback: function(result) {
                                        ...
                                        for (i=0; i<fbPlaces.length; i++) {
                                            places.push({name: fbPlaces[i].name, distance: distance(fbPlaces[i].location.latitude, fbPlaces[i].location.longitude, getLat(), getLon()), lat: fbPlaces[i].location.latitude, lon: fbPlaces[i].location.longitude, category: fbPlaces[i].category, id: fbPlaces[i].id});
                                        }
                                        var placeList = {
                                            itemTpl: '<div class="contact2"><strong>{name}</strong>, {distance} meters</div>',
                                            selModel: {
